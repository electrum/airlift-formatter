 ...
 *******************************************************************************/
package io.airlift.formatter.internal.jdt.linewrap;
...
import io.airlift.formatter.internal.jdt.DefaultCodeFormatterOptions;
import io.airlift.formatter.internal.jdt.DefaultCodeFormatterOptions.Alignment;
import io.airlift.formatter.internal.jdt.Token;
import io.airlift.formatter.internal.jdt.Token.WrapMode;
import io.airlift.formatter.internal.jdt.Token.WrapPolicy;
import io.airlift.formatter.internal.jdt.TokenManager;
import io.airlift.formatter.internal.jdt.TokenTraverser;
    ...
    private void preserveExistingLineBreaks() {
        ...
        this.tm.traverse(0, new TokenTraverser() {
            ...
            protected boolean token(Token token, int index) {
                ...
                // AIRLIFT MODIFICATION: When join_wrapped_lines=false, preserve ALL original
                // line breaks, not just for wrappable tokens. Also store original indentation.
                if (lineBreaks > 1 || index == 0) {
                    token.putLineBreaksBefore(lineBreaks);
                    // Also preserve original indent for gofmt-style behavior
                    if (!this.join_wrapped_lines && lineBreaks > 0) {
                        int originalIndent = WrapPreparator.this.tm.getOriginalIndent(token);
                        if (originalIndent >= 0) {
                            token.setOriginalIndent(originalIndent);
                        }
                    }
                }
                else if (!this.join_wrapped_lines && lineBreaks > 0) {
                    // Preserve single line breaks with original indentation for gofmt-style behavior
                    token.putLineBreaksBefore(lineBreaks);
                    int originalIndent = WrapPreparator.this.tm.getOriginalIndent(token);
                    if (originalIndent >= 0) {
                        token.setOriginalIndent(originalIndent);
                ...
                // AIRLIFT MODIFICATION: Store originalIndent even when getLineBreaksToPreserve returns 0
                // due to setPreserveLineBreaksBefore(false). Check ACTUAL source line breaks instead.
                // This fixes continuation indentation loss for closing braces in nested lambdas
                // where blank line removal (via ~blankLinesBeforeClosingBrace) sets
                // setPreserveLineBreaksBefore(false), which prevented originalIndent from being stored.
                else if (!this.join_wrapped_lines && !token.hasOriginalIndent()) {
                    // Check actual line breaks in source (not filtered by preserve flags)
                    int actualLineBreaks = WrapPreparator.this.tm.countLineBreaksBetween(getPrevious(), token);
