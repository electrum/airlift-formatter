...
import io.airlift.formatter.internal.jdt.linewrap.CommentWrapExecutor;
import io.airlift.formatter.internal.jdt.linewrap.WrapPreparator;
...
public class DefaultCodeFormatter extends CodeFormatter {
    ...
    private void prepareComments() {
        ...
        // AIRLIFT MODIFICATION: Track which comments are processed via AST
        // Eclipse JDT's AST sometimes doesn't include comments in certain contexts
        // (e.g., comments inside lambda expressions in enum constructor arguments).
        // We need to process those comments directly from the token list.
        java.util.Set<Integer> processedCommentPositions = new java.util.HashSet<>();
        for (Comment comment : comments) {
            processedCommentPositions.add(comment.getStartPosition());
            comment.accept(commentsPreparator);
        }

        // AIRLIFT MODIFICATION: Process any line comments that weren't in the AST comment list
        // This handles comments that Eclipse JDT's parser doesn't include in getCommentList(),
        // such as comments inside lambdas in enum constructor arguments.
        for (int i = 0; i < this.tokens.size(); i++) {
            Token token = this.tokens.get(i);
            if (token.tokenType == TokenNameCOMMENT_LINE) {
                if (!processedCommentPositions.contains(token.originalStart)) {
                    commentsPreparator.handleLineComment(i);
                }
            }
        }
