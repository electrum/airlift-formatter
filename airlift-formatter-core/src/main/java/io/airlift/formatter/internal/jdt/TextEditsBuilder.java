...
public class TextEditsBuilder extends TokenTraverser {
    ...
    private void bufferWhitespaceBefore(Token token, int index) {
        if (getLineBreaksBefore() > 0) {
            ...
            if (!bufferAlign(token, index) && isSpaceBefore()) {
                // AIRLIFT MODIFICATION: Output exact space count if preserved (gofmt-style)
                // Check both originalSpacesBefore on current token AND originalSpacesAfter on previous token
                int exactSpaceCount = -1;
                if (token.hasOriginalSpacesBefore()) {
                    exactSpaceCount = token.getOriginalSpacesBefore();
                }
                else if (index > 0) {
                    Token previous = this.tm.get(index - 1);
                    if (previous.hasOriginalSpacesAfter()) {
                        exactSpaceCount = previous.getOriginalSpacesAfter();
                    }
                }

                if (exactSpaceCount > 0) {
                    for (int i = 0; i < exactSpaceCount; i++) {
                        this.buffer.append(' ');
                    }
                }
                else {
    ...
    private void bufferLineSeparator(Token token, boolean emptyLine) {
        ...
        // AIRLIFT MODIFICATION: Also check for unformatted block comments (TokenNameNotAToken)
        // to preserve original indentation inside /* */ comments (gofmt-style)
        boolean isUnformattedBlockComment = token != null && token.tokenType == TokenNameNotAToken;
        this.parent.counter = this.counter;
        this.parent.bufferLineSeparator(null, false);
        // AIRLIFT MODIFICATION: Skip indentation for empty lines in both text blocks AND unformatted block comments
        if (!((isTextBlock || isUnformattedBlockComment) && emptyLine && !this.options.indent_empty_lines))
            this.parent.bufferIndent(this.parent.tm.get(this.parentTokenIndex), this.parentTokenIndex);
        this.counter = this.parent.counter;

        if (isTextBlock)
            return;
        if (isUnformattedBlockComment)
    ...
    private void bufferIndent(Token token, int index) {
        // AIRLIFT MODIFICATION: If token has original indentation set (gofmt-style preservation),
        // use that instead of the calculated indentation.
        if (token.hasOriginalIndent()) {
            int originalIndent = token.getOriginalIndent();
            appendIndentationString(this.buffer, this.options.tab_char, this.options.tab_size, originalIndent, 0);
            return;
        }
    ...
    private void handleSingleLineComment(Token lineComment, int index) {
        ...
        if (structure == null) {
            // AIRLIFT MODIFICATION: For unformatted comments, preserve original whitespace (gofmt-style)
            // bufferWhitespaceBefore() already added the correct spaces to buffer, but we need to
            // check if original whitespace exists and skip past it rather than creating an edit.
            if (lineComment.hasOriginalSpacesBefore() && index > 0) {
                Token previous = this.tm.get(index - 1);
                // Clear buffer and skip past original whitespace to preserve it
                this.buffer.setLength(0);
                this.counter = previous.originalEnd + 1 + lineComment.getOriginalSpacesBefore();
            }
            flushBuffer(lineComment.originalStart);
            this.counter = lineComment.originalEnd + 1;
            return;
        }
        // AIRLIFT MODIFICATION: Handle leading WHITESPACE token for gofmt-style preservation
        // When a WHITESPACE token exists at the start of the structure, we want to preserve
        // the original whitespace from the source. bufferWhitespaceBefore() may have already
        // added a space to the buffer, so we need to discard it (not flush it).
        int startIndex = 0;
        if (structure.get(0).tokenType == TokenNameWHITESPACE) {
            // AIRLIFT MODIFICATION: Preserve original whitespace between code and trailing comment (gofmt-style)
            // bufferWhitespaceBefore() added a standard space to the buffer - we need to discard it
            // and instead preserve the original whitespace from the source.
            //
            // The WHITESPACE token spans from (previous token end + 1) to (comment start - 1).
            // By NOT flushing the buffer and just clearing it, we avoid creating any edit.
            // Then we skip the counter past the original whitespace, preserving it.
            Token wsToken = structure.get(0);
            // Critical: clear buffer WITHOUT flushing - this discards the standard space
            // that bufferWhitespaceBefore() added, without creating an edit
            this.buffer.setLength(0);
            // Skip past the original whitespace to preserve it from source
            this.counter = wsToken.originalEnd + 1;
            startIndex = 1; // Skip the WHITESPACE token in the loop
        }
        else {
            flushBuffer(lineComment.originalStart);
        }

        for (int i = startIndex; i < structure.size(); i++) {
