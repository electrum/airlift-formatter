...
public class WrapExecutor {
    ...
    int getWrapIndent(Token token) {
        ...
        // AIRLIFT MODIFICATION: When join_wrapped_lines=false and token has original
        // indent preserved, use the original indent directly instead of computing from
        // parent. This fixes indentation loss in deeply nested lambdas where parent
        // indent changes between forceContinuousWrapping and getWrapIndent calls.
        if (!this.options.join_wrapped_lines && policy.wrapMode == WrapMode.BLOCK_INDENT) {
